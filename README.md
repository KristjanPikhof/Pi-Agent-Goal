...
| `/goal <objective> [--start]`           | Prepare a structured proposal from plain text before saving the active goal. Interactive Pi with the select UI shows Start, Edit, and Cancel when the proposal generator is available; Edit opens the proposal in the modal editor with the objective and acceptance criteria prefilled. If generation is unavailable, the command falls back to the original objective with no criteria. Recognized flags such as `--replace` and `--start` are stripped from the objective even when they appear before the text. If a goal already exists, interactive Pi asks for confirmation. In non-interactive mode, use `--replace` to replace and `--start` to begin immediately. |
...
## AI-prepared plain goals

Plain `/goal` text is treated as a draft, not as an already-active work order. When Pi has a proposal generator, the extension asks it to normalize the text into:

- an objective with the same meaning and boundaries as the user's text
- acceptance criteria, but only when they directly follow from that text

In interactive Pi with the select UI, the user reviews the generated proposal before anything is saved. The choices are:

- **Start**, save the prepared goal and queue the one-shot start handoff.
- **Edit**, open a prefilled modal editor for the objective and acceptance criteria, then return to review.
- **Cancel**, discard the unsaved proposal.

This is different from a binary "start or don't start" flow. The user can correct the objective and criteria before activation, so the model has clearer completion checks once work begins. It also avoids inventing criteria after the goal is already active. If no criteria are available, the stored goal stays criteria-free and start/continuation prompts say that no acceptance criteria were specified, with the objective as the source of truth.

The generator is optional. If Pi does not provide one, or if generation fails, `/goal <objective>` keeps the original objective, stores no criteria, and warns that the goal is criteria-free.
